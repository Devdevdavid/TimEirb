...
Testbench::Testbench(sc_module_name name)
  ...
  pmcSocket.bind(timer1->pmcSocket);
  busSocket.bind(timer1->busSocket);
...
void Testbench::pmc_write(struct pmc_data pmcData) {
  tlm_generic_payload *trans = new tlm_generic_payload;

  trans->set_command(TLM_WRITE_COMMAND);
  trans->set_address(0);
  trans->set_data_ptr(reinterpret_cast<unsigned char *>(&pmcData));
